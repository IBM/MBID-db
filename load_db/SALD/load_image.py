...
if __name__ == '__main__':
    ...
    with app.app_context():
        dataset_object = SourceDataset.query.filter(SourceDataset.designation == 'SALD').first()
        ...
        for i in bucket.objects.filter(Prefix=prefix):
            if (i.key.endswith("nii.gz")):
                ...
                try:
                    ...
                    continue
                # SALD Dataset has two types of MRI for every patient, T1 and rsfMRI
                if ("T1w" in i.key):
                    image_type = 'T1'
                elif "task-rest" in i.key:
                    image_type = 'rsfMRI'
                else:
                    print("ERROR: Unable to select image type")
                # SALD Dataset is all about age, so there are no symptoms, conditions or bmi information
                ...
                try:
                    ...
                    subject_external_id = re.search(re.compile('sub-\d{6}'), i.key).group(0).replace("sub-0", "")
                except:
                    print("ERROR: Unable to extract external id from file")

                # Visit parameters: Sald has no visit, os adding sald_visit as external id
                visit_external_id = '1'
                ...
                if existing_subject:
                    ...
                    if existing_image:
                        ...
                        existing_image.metadata_json = json_filename
                        ...
                        image_db = Image(visit_id=existing_visit.id, subject_id=existing_subject.id,
                                            ...
                                            type=image_type, metadata_json=json_filename,
                                            days_since_baseline=visit_days_since_baseline,
